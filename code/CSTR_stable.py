...
def cost_function_stable(u, x0, N, dt):
    ...
    total_cost = 0
    for i in range(N):
        state = odeint(cstr_dynamics_stable, state, [0, dt], args=(u[i],))[-1]
...
def simulate_encrypted_mpc_stable(initial_state, time_horizon=5.0, dt=0.1):
    ...
    for t in times[:-1]:
        ...
        control_input = mpc_optimization_stable(decrypted_state, dt=dt)
        controls.append(control_input)
        state = odeint(cstr_dynamics_stable, state, [t, t + dt], args=(control_input,))[-1]
...
def plot_state_trajectories(states, times):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('Stable_trajectory.png')
